...
retry:
  ...
  if(res) {
    fprintf(stderr, "%s:%d curl_ws_send() failed with code %d (%s)\n",
...
retry:
  ...
  if(res) {
    fprintf(stderr, "%s:%d curl_ws_recv() failed with code %d (%s)\n",
            __FILE__, __LINE__, res, curl_easy_strerror(res));
    assert(nread == 0);
    return res;
  }
  ...
  *bytesleft = meta->bytesleft;
  ...
  fprintf(stdout, "%s [%" CURL_FORMAT_CURL_OFF_T "]",
          descr_flags(meta->flags), meta->bytesleft);
